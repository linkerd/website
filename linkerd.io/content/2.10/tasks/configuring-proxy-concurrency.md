...
command](/2.10/tasks/install/), the `--proxy-cpu-limit` argument sets this
...
[injectable Kubernetes resource](/2.10/proxy-injection/), such as a namespace, pod,
or deployment.
...
When using [Helm](/2.10/tasks/install-helm/), users must take care to set the
