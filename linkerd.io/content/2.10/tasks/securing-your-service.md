...
(mTLS)](/2.10/features/automatic-mtls/) for TCP traffic between meshed pods, by
establishing and authenticating secure, private TLS connections between Linkerd
proxies. Simply [add your services](/2.10/tasks/adding-your-service/) to Linkerd,
...
viz edges`](/2.10/reference/cli/edges/) command.  For example:
...
documentation](/2.10/features/automatic-mtls/) for more on what these identities
...
[`linkerd viz tap` command](/2.10/reference/cli/tap/) to sample real time request data.
...
Linkerd includes a [debug sidecar](/2.10/tasks/using-the-debug-container/) that
...
application](/2.10/getting-started/), we can add the debug sidecar by running:
...
documentation](/2.10/features/automatic-mtls/)---so if you are relying on Linkerd
