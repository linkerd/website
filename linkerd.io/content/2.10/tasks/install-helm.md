...
provide your own, or follow [these instructions](generate-certificates/) to
generate new ones.
...
If installing with CNI, make sure that you add the `--set cniEnabled=true` flag
to your `helm install` command.

## Setting High-Availability

The chart contains a file `values-ha.yaml` that overrides some default values as
...
memory/cpu limits and affinities are specified in that file.
...
To install Linkerd to a different namespace than the default `linkerd`, override
...
control plane to work correctly. This means that the chart won't work with Helm
v2's `--namespace` option. If you're relying on a separate tool to create the
...
behaviour. The ones that special attention should be paid to are
