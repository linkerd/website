...
the new anchor. The following command uses `kubectl` to fetch the Linkerd
config, `jq`/[`yq`](https://github.com/mikefarah/yq) to extract the current
trust anchor, and `step` to combine it with the newly generated trust anchor:
...
start of this process. Additionally, you can use the `rollout restart` command
to bring the configuration of your other injected resources up to date:
