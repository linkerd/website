...
`ServerAuthorization` that can target [`HTTPRoute`]s as well as `Server`s. Use
of `AuthorizationPolicy` is preferred, and `ServerAuthorization` will be
...
configured for a port, or when a [`Server`] is configured but no [`HTTPRoute`]s
are configured for that [`Server`]. If any [`HTTPRoute`] matches the `Server`,
these automatic authorizations are not created and you must explicitly create
them for health and readiness probes.
