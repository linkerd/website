...
Finally, you'll "mesh" a application by adding Linkerd's _data plane_ to it.
...
If you're using a GKE "private cluster" or Calico CNI, there are some
...
Piping this manifest into `kubectl apply` then instructs Kubernetes to add those
...
[using Helm charts](../tasks/install-helm/); by using a
[multi-stage install](../tasks/install/#multi-stage-install) for clusters with
strict security policies; or by using a marketplace install from your Kubernetes
provider.
