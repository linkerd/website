...
Linkerd uses a sophisticated [EWMA load balancing algorithm](../features/load-balancing.md)
which typically requires no configuration or tuning. However, it also supports
special handling for backends which support rate limiting. This special handling
can be enabled with Service annotations.
