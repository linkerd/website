...
2. Use [automatic proxy injection](#method-2-automatic-proxy-injection) to do this automatically
...
To add the Linkerd data plane proxies to a Kubernetes resource, run:
...
the availability of the application will not be affected. When you add the
...
It doesn't work with other workloads like StatefulSet, DaemonSet, Pod etc.

An alternative to using the CLI to add the Linkerd data plane proxies to a
resource is to do it on the cluster, using a Kubernetes feature called
[admission
...
* Namespaces that are labeled with the `linkerd.io/auto-inject: disabled` label are ignored.
